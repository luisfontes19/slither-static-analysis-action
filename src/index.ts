...
const failOnInformativeResults =
  parseInt(core.getInput("informative-threshold")) || 10;
const failOnOptimizationResults =
  parseInt(core.getInput("optimization-threshold")) || 1;
...
const prepare = async () => {
  ...
  await exec.exec(
    "sudo apt-get install -y git python3 python3-setuptools wget software-properties-common"
  );

  core.info("Downloading slither");
  await exec.exec(
    `wget https://github.com/crytic/slither/archive/${slitherVersion}.zip -O /tmp/slither.zip`
  );
  ...
  await exec.exec("sudo python3 setup.py install", undefined, {
    cwd: `slither-${slitherVersion}`,
  });
...
const runSlither = async (): Promise<string> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    exec
      .exec(
        `slither --json - ${projectPath} ` + slitherParams,
        undefined,
        options
      )
      .then(() => resolve(output))
...
const showStats = (counts: any) => {
  core.warning(
    `Stats: High: ${counts["High"]}, Medium: ${counts["Medium"]}, Low: ${counts["Low"]}, Informative: ${counts["Informational"]}, Optimizations: ${counts["Optimization"]}`
  );

  if (failOnHighResults !== 0 && failOnHighResults <= counts["High"])
    core.setFailed(
      `Number of High results is equal or bigger then the defined threshold of ${failOnHighResults}`
    );
  else if (failOnMediumResults !== 0 && failOnMediumResults <= counts["Medium"])
    core.setFailed(
      `Number of Medium results is equal or bigger then the defined threshold of ${failOnMediumResults}`
    );
  else if (failOnLowResults !== 0 && failOnLowResults <= counts["Low"])
    core.setFailed(
      `Number of Low results is equal or bigger then the defined threshold of ${failOnLowResults}`
    );
  else if (
    failOnInformativeResults !== 0 &&
    failOnInformativeResults <= counts["Informational"]
  )
    core.setFailed(
      `Number of Informative results is equal or bigger then the defined threshold of ${failOnInformativeResults}`
    );
  else if (
    failOnOptimizationResults !== 0 &&
    failOnOptimizationResults <= counts["Optimization"]
  )
    core.setFailed(
      `Number of Optimization results is equal or bigger then the defined threshold of ${failOnOptimizationResults}`
    );
};
